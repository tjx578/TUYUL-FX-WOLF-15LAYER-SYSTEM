...
=============================================
Thin façade over ``NewsService``.  All provider fetching, caching, and
blocker logic lives in the service layer — this module is routing only.

Endpoints:
  GET  /api/v1/calendar                   → Today's economic events
  GET  /api/v1/calendar/upcoming          → Upcoming events within N hours
  GET  /api/v1/calendar/blocker           → Blocker status for a symbol
  GET  /api/v1/calendar/health            → Source health
  POST /api/v1/calendar/news-lock/enable  → Toggle news lock (manual override)
  POST /api/v1/calendar/news-lock/disable → Remove manual news lock
...
async def _get_news_service():
    ...
    We do not cache the service instance across requests because the
    Redis client is request-scoped via ``get_async_redis()``.
    """
...
async def get_calendar(
    ...
    # Annotate with minutes_away and is_imminent
...
async def blocker_status(
    ...
    Combines the automated engine result with the manual lock override.
    """
    service = await _get_news_service()
    r: aioredis.Redis = await get_async_redis()
    ...
    # Manual lock overrides automated engine
