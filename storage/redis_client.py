...
# Type alias for Redis Streams read results:
# list of (stream_name, list of (entry_id, fields)) tuples
StreamReadResult = list[tuple[str, list[tuple[str, dict[str, str]]]]]
...
class RedisClient:
    ...
    def ping(self) -> bool:
        ...
        """
        result = self.client.ping()  # type: ignore[union-attr]
        return bool(result)
    ...
    def xread(
        ...
        """
        return cast(StreamReadResult, self.client.xread(streams, count=count, block=block)) # type: ignore
    ...
    def xreadgroup(
        ...
        Returns:
            List of [stream_name, [(entry_id, fields), ...]] tuples.
        """
        return cast(StreamReadResult, self.client.xreadgroup(groupname, consumername, streams, count=count, block=block)) # type: ignore
