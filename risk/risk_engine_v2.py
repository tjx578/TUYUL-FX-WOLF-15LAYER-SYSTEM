...
class RiskEngineV2:
    ...
    def evaluate(
        ...
        try:
            lots = self._calculate_lots(
                signal=signal, profile=profile, vix_level=vix_level, session=session
            )
        except (InvalidPositionSize, RiskCalculationError) as exc:
            logger.warning(
                "Trade DENIED: position sizing failed", account_id=self._account_id, error=str(exc)
            )
            return RiskEvalResult(
                verdict=RiskVerdict.DENY,
    ...
    def _calculate_pip_value(
        self, lot_size: float, pips_at_risk: float, risk_amount: float
    ) -> float:
        """
    ...
    def get_account_snapshot(
        self, vix_level: float | None = None, session: str | None = None
    ) -> dict:
        profile = load_risk_profile(self._account_id)
