...
def _maybe_emit_structure_ladder_diagnostic(
    ...
    *,
    ...
    payload = {
        ...
        "htf_snapshot_enabled": os.getenv("HTF_STRUCTURE_SNAPSHOT_ENABLED", "true").strip().lower() == "true",
        "htf_daily_phase_feed_enabled": os.getenv("HTF_DAILY_PHASE_FEED_ENABLED", "true").strip().lower() == "true",
...
def _breakout_continuation_levels(
    *,
    ...
    sl_safe = _round_price(entry + stop_sign * risk)
    tp1_rr_level = max(min_rr, tp1_rr)
    tp1 = _round_price(entry + sign * risk * tp1_rr_level)
    tp2 = _round_price(entry + sign * risk * (tp1_rr_level + 1.0))
    tp3 = _round_price(entry + sign * risk * (tp1_rr_level + 2.0))
    tp4 = _round_price(entry + sign * risk * (tp1_rr_level + 3.0))
    return {
        "sl_safe": sl_safe,
        "tp1": tp1,
        "tp2": tp2,
        ...
        "tp_min_rr": tp1,
        "tp1_rr": tp1_rr_level,
        "tp2_rr": tp1_rr_level + 1.0,
        "tp3_rr": tp1_rr_level + 2.0,
        "tp4_rr": tp1_rr_level + 3.0,
        "trade_plan": {
            "direction": direction,
            "entry_mode": f"{direction}_BREAKOUT_RETEST",
...
def _rr_fallback_target_result(
    *,
    ...
    sign = -1.0 if direction == "SELL" else 1.0
    tp1 = _round_price(entry + sign * risk * min_rr)
    tp2 = _round_price(entry + sign * risk * (min_rr + 1.0))
    tp3 = _round_price(entry + sign * risk * (min_rr + 2.0))
    tp4 = _round_price(entry + sign * risk * (min_rr + 3.0))
    return {
        **common,
        "target_mode": "PROVISIONAL_RR_FALLBACK",
        "target_source": "rr_projection_only",
