...
def build_structural_proof_v31(
    *,
    ...
    # Context gates (#495). Counter-pressure is deferred by design and never falls back to continuation.
    if evaluation.outcome == "AUTHORIZE_PROOF_REQUIRED_COUNTER_PRESSURE":
        return _no("COUNTER_PRESSURE_PROOF_NOT_IMPLEMENTED_BY_DESIGN")
    if evaluation.outcome == "DEFER":
        return _no("CONTEXT_DEFERRED_NO_PROOF_PROMOTION")
    if evaluation.outcome != "ALIGN" or evaluation.selected_route is None:
        return _no("CONTEXT_ROUTE_NOT_PERMITTED")
