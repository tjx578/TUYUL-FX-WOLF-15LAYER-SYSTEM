...
from ._types import DEFAULT_PRECISION_WEIGHT_MAX, DEFAULT_PRECISION_WEIGHT_MIN, ConfidenceLineage
...
class FusionIntegrator:
    ...
    def fuse_reflective_context(
        self, *, market_data: dict[str, Any] | None = None, coherence_audit: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        lineage = ConfidenceLineage(
            raw=round(c12_raw, 4),
            weighted=round(c12_w, 4),
            final=round(c12_w, 4),
            precision_weight=round(pw, 4),
            gate_threshold=self.gate_threshold,
            gate_pass=True,
            authority="FusionIntegrator",
