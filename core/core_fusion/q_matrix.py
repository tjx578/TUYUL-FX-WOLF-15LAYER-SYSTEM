...
class QMatrixGenerator:
    ...
    def _calculate_base_rates(self) -> List[List[float]]:
        ...
        S = TransitionState
        bt = {
            (S.STRONG_BULLISH, S.WEAK_BULLISH): 0.25, (S.STRONG_BULLISH, S.NEUTRAL): 0.10,
            (S.STRONG_BULLISH, S.HIGH_VOLATILITY): 0.05, (S.WEAK_BULLISH, S.STRONG_BULLISH): 0.20,
            (S.WEAK_BULLISH, S.NEUTRAL): 0.30, (S.WEAK_BULLISH, S.WEAK_BEARISH): 0.15,
            (S.NEUTRAL, S.WEAK_BULLISH): 0.25, (S.NEUTRAL, S.WEAK_BEARISH): 0.25,
            (S.NEUTRAL, S.LOW_VOLATILITY): 0.10, (S.WEAK_BEARISH, S.STRONG_BEARISH): 0.20,
            (S.WEAK_BEARISH, S.NEUTRAL): 0.30, (S.WEAK_BEARISH, S.WEAK_BULLISH): 0.15,
            (S.STRONG_BEARISH, S.WEAK_BEARISH): 0.25, (S.STRONG_BEARISH, S.NEUTRAL): 0.10,
            (S.STRONG_BEARISH, S.HIGH_VOLATILITY): 0.05, (S.HIGH_VOLATILITY, S.STRONG_BULLISH): 0.15,
            (S.HIGH_VOLATILITY, S.STRONG_BEARISH): 0.15, (S.HIGH_VOLATILITY, S.NEUTRAL): 0.20,
            (S.LOW_VOLATILITY, S.NEUTRAL): 0.30, (S.LOW_VOLATILITY, S.WEAK_BULLISH): 0.15,
    ...
    def generate(self, market_data: Dict[str, float]) -> List[List[float]]:
        ...
        if vol > 1.5:
            for i in range(self.n_states): rates[i][hv] *= vf; rates[lv][i] *= vf
        elif vol < 0.5:
            for i in range(self.n_states): rates[i][lv] *= (2 - vf); rates[hv][i] *= (2 - vf)

        if ts > 0.5:
            sb = self.state_to_idx[TransitionState.STRONG_BULLISH]
            for i in range(self.n_states): rates[i][sb] *= (1 + ts * self.config.trend_sensitivity)
        elif ts < -0.5:
            sbe = self.state_to_idx[TransitionState.STRONG_BEARISH]
            for i in range(self.n_states): rates[i][sbe] *= (1 + abs(ts) * self.config.trend_sensitivity)

        for i in range(self.n_states):
            rs = 0.0
            for j in range(self.n_states):
                if i != j: rates[i][j] += self.config.regularization; rs += rates[i][j]
            rates[i][i] = -rs
        self.q_matrix = rates
        return rates

    def get_escape_rate(self, state: TransitionState) -> float:
        if self.q_matrix is None: raise ValueError("Call generate() first.")
    ...
    def export_matrix(self) -> Dict[str, Any]:
        if self.q_matrix is None: return {}
        return {"states": [s.value for s in self.states], "matrix": self.q_matrix,
