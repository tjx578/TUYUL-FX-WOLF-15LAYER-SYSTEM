...
from ._types import CoherenceAudit, MTF_TIMEFRAMES
from ._utils import _clamp01


def multi_timeframe_alignment_analyzer(
    biases: Dict[str, float], rsi_values: Dict[str, float],
    reflective_intensity: float = 1.0, trq_energy: float = 1.0,
    ...
    if err: return {"status": "Invalid timeframe data", "detail": err}
    ...
    if bs >= 0.85: rs = "Strong Alignment"
    elif bs >= 0.65: rs = "Moderate Alignment"
    elif bs >= 0.45: rs = "Weak Alignment"
    else: rs = "Disaligned"
    ...
    return {
        ...
        "alignment_ratio": round(ar, 3), "rsi_variance": round(rv, 3),
        "rsi_coherence": round(rc, 3), "bias_strength": round(bs, 3),
        "meta_sync": round((alpha + beta + gamma) / 3, 3),
        "reflective_intensity": round(reflective_intensity, 3),
        "trq_energy": round(trq_energy, 3), "integrity_index": round(integrity_index, 3),
        "time_coherence_index": round(tci, 3), "regime_state": rs,
        "status": "ok", "symbol": symbol, "pair": pair, "trade_id": trade_id,
    }


def _validate_mtf_inputs(biases: Dict[str, float], rsi_values: Dict[str, float],
                          timeframes: Iterable[str]) -> Optional[str]:
    tfs = list(timeframes)
    mb = [tf for tf in tfs if tf not in biases]; mr = [tf for tf in tfs if tf not in rsi_values]
    if mb or mr: return f"Missing data for: {mb + mr}"
...
def audit_reflective_coherence(*, mtf_data: Optional[List[Dict[str, Any]]] = None,
                                lookback: int = 64, divergence_threshold: float = 0.22,
                                gate_threshold: float = 0.96) -> Dict[str, Any]:
    """Audit reflective coherence from MTF alignment data."""
    ts = datetime.now(timezone.utc).isoformat()
    default = CoherenceAudit(timestamp=ts, lookback=lookback, reflective_coherence=0.97,
        divergence_window=False, divergence_alert="✅ Default Stable",
        stability_state="Stable", gate_threshold=gate_threshold, gate_pass=True).as_dict()
    ...
    if not ci:
        default["divergence_alert"] = "✅ No data"; return default
    ...
    rc = avg_c
    if rc > 1.5: rc = rc / 100.0
    rc = _clamp01(rc)

    result = CoherenceAudit(timestamp=ts, lookback=lookback, reflective_coherence=round(rc, 4),
        divergence_window=dw, divergence_alert="⚠️ MTF Divergence Detected" if dw else "✅ Stable Alignment",
        stability_state="Stable" if rc >= gate_threshold else "Degraded",
        gate_threshold=gate_threshold, gate_pass=rc >= gate_threshold).as_dict()
