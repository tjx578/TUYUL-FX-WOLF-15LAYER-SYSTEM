...
from ._utils import _clamp, _clamp01, _inputs_valid, _safe_float
...
class FusionPrecisionEngine:
    ...
    def compute_precision(
        ...
        *,
        ...
        if (
            not _inputs_valid(price, ema_fast_val, ema_slow_val, vwap, atr, reflex_strength, volatility, rsi)
            or atr <= 0
        ):
        ...
        bias = (
            FusionBiasMode.BULLISH.value
            if fusion > 0.25
            else FusionBiasMode.BEARISH.value
            if fusion < -0.25
            else FusionBiasMode.NEUTRAL.value
        )
        hint = _clamp(abs(fusion) * 0.85, 0.0, 1.0)
        rsi_bonus = 0.10 if (rsi >= 65 and fusion > 0) or (rsi <= 35 and fusion < 0) else 0.0
        pw = _clamp(
            1.0 + (_clamp(abs(fusion), 0, 1) - 0.35) * 0.45 + rsi_bonus,
            DEFAULT_PRECISION_WEIGHT_MIN,
            DEFAULT_PRECISION_WEIGHT_MAX,
        )
        ...
        return FusionPrecisionResult(
            timestamp=ts,
            fusion_strength=round(fusion, 6),
            bias_mode=bias,
            precision_weight=round(float(pw), 4),
            precision_confidence_hint=round(float(hint), 4),
            details={
                "ema_strength": round(float(ema_str), 6),
                "vwap_signal": round(float(vwap_sig), 6),
                "reflex_strength": round(float(ref_w), 6),
                "volatility_adj": round(float(vol_adj), 6),
                "rsi": float(rsi),
            },
            symbol=symbol,
            pair=pair,
...
def evaluate_fusion_metrics(fusion_data: dict[str, Any], threshold: float = 0.75) -> dict[str, Any]:
    ...
    action = (
        ...
        else FusionAction.MONITOR.value
        if valid and score >= 0.6
        else FusionAction.WAIT.value
    )
    return {
        "confidence": conf,
        "direction": direction,
