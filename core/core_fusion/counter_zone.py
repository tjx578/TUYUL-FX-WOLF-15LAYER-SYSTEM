...
def _compute_confidence(ctx: CounterZoneContext) -> float:
    ...
    ms = _clamp(
        (abs(ctx.rsi - 50) / 50 + abs(ctx.mfi - 50) / 50 + min(abs(ctx.cci50) / 200, 1) + abs(ctx.rsi_h4 - 50) / 50)
        / 4,
        0.0,
        1.0,
    )
...
def _derive_direction(ctx: CounterZoneContext) -> str:
    if ctx.price > ctx.vwap and ctx.rsi >= 60:
        return "SELL"
    if ctx.price < ctx.vwap and ctx.rsi <= 40:
        return "BUY"
    if ctx.mfi <= 45 and ctx.price <= ctx.vwap:
        return "BUY"
    if ctx.rsi_h4 >= 65:
        return "SELL"
...
def smart_money_counter_v3_5_reflective(
    *,
    ...
    vd = abs(ctx.price - ctx.vwap)
    spread = abs(ctx.mfi - ctx.cci50)
    vt = 1.2 * ctx.atr
    re = ctx.trq_energy * ctx.reflective_intensity
    est = min(55.0, 40.0)
    if not (vd >= vt and spread >= est and re >= 0.85):
        return {"status": "No valid counter-zone", "confidence": 0.0, "counter_zone": False}

    d = _derive_direction(ctx)
    rb = max(ctx.atr * 1.6, 0.0008)
    tb = max(ctx.atr * 1.8, 0.0012)
    if d == "BUY":
