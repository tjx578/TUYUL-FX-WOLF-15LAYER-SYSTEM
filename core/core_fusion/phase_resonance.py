...
def phase_resonance_engine_v1_5(
    ...
    alpha_drift: float = 0.0, beta_drift: float = 0.0, gamma_drift: float = 0.0,
    ...
    trq_energy = _clamp(trq_energy, 0.1, 10.0); reflective_intensity = _clamp(reflective_intensity, 0.1, 10.0)
    alpha = _clamp(alpha, 0.5, 2.0); beta = _clamp(beta, 0.5, 2.0); gamma = _clamp(gamma, 0.5, 2.0)
    alpha_drift = _clamp(alpha_drift, -0.5, 0.5); beta_drift = _clamp(beta_drift, -0.5, 0.5)
    gamma_drift = _clamp(gamma_drift, -0.5, 0.5); integrity_index = _clamp01(integrity_index)

    pe = abs(price_change / atr); ve = math.log1p(volume_change); te = math.log1p(time_delta)
    eb = (pe + ve + te) / 3
    ...
    if pri >= 1.3: fs = ResonanceState.EXPANSION_RESONANCE.value
    elif pri >= 0.9: fs = ResonanceState.EQUILIBRIUM_RESONANCE.value
    elif pri >= 0.7: fs = ResonanceState.ADAPTIVE_COMPRESSION.value
    else: fs = ResonanceState.PHASE_DRIFT_DETECTED.value
    ...
    return {
        ...
        "price_energy": round(pe, 3), "volume_energy": round(ve, 3), "time_energy": round(te, 3),
        "energy_balance": round(eb, 3), "imbalance_factor": round(ib, 3),
        "phase_resonance_index": round(pri, 3), "resonance_state": fs,
        "alpha": round(alpha, 3), "beta": round(beta, 3), "gamma": round(gamma, 3),
        "alpha_drift": round(alpha_drift, 4), "beta_drift": round(beta_drift, 4), "gamma_drift": round(gamma_drift, 4),
        "drift_correction": round(dc, 3), "reflective_intensity": round(reflective_intensity, 3),
        "trq_energy": round(trq_energy, 3), "integrity_index": round(integrity_index, 3),
        "coherence_score": round(cs, 3), "status": "ok",
