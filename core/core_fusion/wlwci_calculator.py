...
WLWCI_CONFIG: Final[Dict[str, Any]] = {
    ...
    "micro_bounds": {"twms_micro_max": 0.30, "twms_micro_min": -0.30, "volatility_cap": 0.50, "min_confidence_multiplier": 0.70},
    "volatility_regimes": {
    ...
    "micro_rules": {"require_macro_alignment": True, "macro_alignment_threshold": 0.50,
                    "disable_on_extreme_vol": True, "conflict_decay_factor": 0.50},
    "fallback": {
...
def calculate_wlwci(twms_macro: float, trend_fusion: float, twms_micro: float,
                    volatility: float, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    cfg = config or WLWCI_CONFIG; w = cfg["weights"]; b = cfg["micro_bounds"]; r = cfg["volatility_regimes"]
    tmc = _clamp(twms_micro, b["twms_micro_min"], b["twms_micro_max"])
    vb, regime = 1.0, "medium"
    for rn, rc in r.items():
        if volatility <= rc["threshold"]: vb = rc["confidence_boost"]; regime = rn; break
    vp = min(volatility, b["volatility_cap"])
    raw = w["twms_macro"] * twms_macro + w["trend_fusion"] * trend_fusion + w["twms_micro"] * tmc - w["volatility_penalty"] * vp
    final = _clamp01(raw * vb)
    return {"wlwci": round(final, 4), "wlwci_raw": round(raw, 4), "volatility_regime": regime,
            "confidence_boost": vb, "components": {
