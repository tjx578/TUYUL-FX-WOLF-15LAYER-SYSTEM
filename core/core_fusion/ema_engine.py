...
class EMAFusionEngine:
    def __init__(self, periods: Optional[List[int]] = None, smoothing: float = 2.0) -> None:
    ...
    def compute(self, prices: List[float]) -> Dict[str, Any]:
        ...
        return {**emas, "direction": self._direction(emas),
                ...
                "price": prices[-1], "timestamp": datetime.now(timezone.utc).isoformat()}
    ...
    def _direction(self, emas: Dict[str, float]) -> str:
        ...
        if len(vals) < 2: return "NEUTRAL"
        if all(vals[i] >= vals[i+1] for i in range(len(vals)-1)): return "BULL"
        if all(vals[i] <= vals[i+1] for i in range(len(vals)-1)): return "BEAR"
        return "NEUTRAL"

    def _strength(self, emas: Dict[str, float], price: float) -> float:
        if not emas: return 0.5
...
class MultiEMAFusion:
    ...
    def integrate(self, closes: List[float], wlwci: float = 0.88) -> Dict[str, Any]:
        if not closes: return {"status": "no_data", "fusion_strength": 0.5}
        results: Dict[str, Any] = {}; slopes: List[float] = []
        for period in self.ema_periods:
            ...
            if vals:
                ...
                if len(vals) > 1 and vals[-2] != 0:
                    ...
                    results[f"{key}_slope"] = round(s, 6); slopes.append(s)
                else: results[f"{key}_slope"] = 0.0
            else: results[key] = 0.0; results[f"{key}_slope"] = 0.0
