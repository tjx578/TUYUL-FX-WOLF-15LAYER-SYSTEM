        ...
        <div key={eaClass} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div
            style={{
              ...
              color: CLASS_COLORS[eaClass] ?? "var(--text-muted)",
              display: "flex",
              alignItems: "center",
              gap: 6,
            }}
          >
            <span
              style={{
                ...
                height: 6,
                borderRadius: "50%",
                background: CLASS_COLORS[eaClass] ?? "var(--text-muted)",
            ...
            return (
              ...
              >
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                  ...
                  <DetailRow
                    label="Risk ×"
                    value={String(profile.default_risk_multiplier)}
                  />
                  {profile.default_news_lock && (
                    <DetailRow label="News Lock" value={profile.default_news_lock} />
                  )}
                  {profile.allowed_strategies.length > 0 && (
                    <DetailRow
                      label="Strategies"
                      value={profile.allowed_strategies.join(", ")}
                    />
                  )}
                </div>
              </div>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
      ...
      <span className="num" style={{ fontWeight: 600, color: "var(--text-secondary)" }}>
        {value}
      </span>
    </div>
  );
