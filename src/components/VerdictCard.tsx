...
export function VerdictCard({
    ...
    const panelGlow = verdictGlow(v, selected ?? false);

    const panelStyle: CSSProperties =
        panelGlow === "cyan"
            ? { boxShadow: "0 0 0 1px rgba(0, 220, 255, 0.35), var(--shadow-card)" }
            : panelGlow === "emerald"
                ? { boxShadow: "0 0 0 1px rgba(0, 245, 160, 0.25), var(--shadow-card)" }
                : panelGlow === "orange"
                    ? { boxShadow: "0 0 0 1px rgba(255, 164, 32, 0.28), var(--shadow-card)" }
                    : {};
