...
export default function SettingsPage() {
  ...
  const activate = async () => {
    try {
      await sendWrite(API_ENDPOINTS.configActive, "POST", { profile_name: profileName });
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div className="rounded-xl border p-4" style={{ display: "grid", gap: 8 }}>
        <div className="font-semibold">SETTINGS CENTER (Single Source of Truth)</div>
        <div className="text-sm">API Health: {data?.status ?? "unknown"}</div>
        <div className="text-xs opacity-80">Endpoints: {endpointList.join(" • ")}</div>
        {message && <div className="text-xs" style={{ color: "#7dd3fc" }}>{message}</div>}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 12 }}>
        <div className="rounded-xl border p-2" style={{ display: "grid", gap: 6, alignContent: "start" }}>
          {TABS.map((item) => (
            <button
              key={item}
              onClick={() => setTab(item)}
        ...
        <div className="rounded-xl border p-4" style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "end" }}>
              <button className="rounded-lg border px-3 py-2" onClick={activate}>Set Active Profile</button>
              <button className="rounded-lg border px-3 py-2" onClick={() => lockConfig(true)}>Lock Config</button>
              <button className="rounded-lg border px-3 py-2" onClick={() => lockConfig(false)}>Unlock</button>
            </div>
          </div>

          <div className="text-xs opacity-80">
            Scope hierarchy: <b>global → account → prop_firm → pair</b>. Merge dilakukan backend.
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="rounded-lg border px-3 py-2" onClick={saveOverride}>Save Changes</button>
            <button className="rounded-lg border px-3 py-2" onClick={() => setOverrideJson('{"settings":{}}')}>Reset JSON</button>
          </div>
        </div>
      </div>
    </div>
