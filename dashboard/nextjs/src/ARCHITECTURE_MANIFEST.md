# Architecture Manifest — V2 (22 → 6 Pages)

> Single source of truth for folder ownership, import rules, and migration status.
> Updated: Architecture V2 — consolidated from `(root)` + `(control)` into single `(main)` route group.
...
| ------ | ---- | ------------ |
| `app/(main)/page.tsx` | **Dashboard** — root page, thin delegation | MAY import: `features/command/CommandCenterScreen` only |
| `app/(main)/*/page.tsx` | **Route entry** — tab container with feature screens | MAY import: `features/*/components/*Screen` only |
| `app/(main)/*/layout.tsx` | **Route metadata** — title, description | No business imports |
| `app/(main)/layout.tsx` | **Shell layout** — auth + DashboardShell wrapper | Server component, `force-dynamic` |
...
## Route Architecture (V2)

### 6 pages under `(main)` ✅

| Route | Page | Feature Screens (tabs) | Status |
| ----- | ---- | ---------------------- | ------ |
| `/` | Dashboard | `CommandCenterScreen` | ✅ Live |
| `/signals` | Signals | `SignalBoardScreen` + History/Pipeline tabs | ✅ Live |
| `/trades` | Trades | `TradesScreen`, `JournalScreen` + History/Exposure tabs | ✅ Live |
| `/risk` | Risk & Compliance | `RiskScreen`, `AccountsScreen`, `PropFirmScreen` | ✅ Live |
| `/market` | Market | TradingView Chart/Calendar, `NewsScreen`, Watchlist | ✅ Live |
| `/settings` | Settings & Operations | `SettingsScreen`, `AgentManagerScreen` + Audit (role-gated) | ✅ Live |

### Deleted route groups

| Route Group | Status | Notes |
| ----------- | ------ | ----- |
| `(root)/*` | ❌ Deleted | All legacy routes, aliases, redirect stubs removed |
| `(control)/*` | ❌ Deleted | Consolidated into `(main)` |
| `(admin)/*` | ❌ Deleted | Admin features moved to Settings (role-gated) |
...
`app/(main)/*/page.tsx` — only `@/features/*/components/*Screen` imports allowed.
...
5. **Consolidate stores** — evaluate Auth+Session+Authority merge
6. **Build placeholder tab panels** for History, Pipeline, Exposure tabs
