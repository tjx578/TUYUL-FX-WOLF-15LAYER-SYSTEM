...
export function PropFirmScreen() {
  const { data: accounts } = useAccounts();
  const [selectedAccountId, setSelectedAccountId] = useState("");
  const activeAccountId = selectedAccountId || accounts?.[0]?.account_id || "";

  const { data: status } = usePropFirmStatus(activeAccountId) as {
    data: PropFirmStatus | undefined;
  };
  const { data: phase } = usePropFirmPhase(activeAccountId) as {
    data: PropFirmPhase | undefined;
  };
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <DomainHeader
        domain="prop-firm"
        title="PROP FIRM CONTROL"
        subtitle="Compliance snapshot, phase progress, and account-level enforcement"
      />

      {accounts && accounts.length > 1 && (
        <div className="rounded-xl border p-4">
          <label className="font-semibold" htmlFor="account-select">
            Account
          </label>
          <select
            id="account-select"
