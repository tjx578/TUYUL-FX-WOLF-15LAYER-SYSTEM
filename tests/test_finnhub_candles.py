...
class TestPremiumFallback:
    ...
    @patch("ingest.finnhub_candles.FinnhubCandleFetcher.fetch")
    async def test_premium_error_triggers_fallback(self, mock_fetch: AsyncMock, mock_fallback: AsyncMock) -> None:
    ...
    @patch("ingest.finnhub_candles.FinnhubCandleFetcher.try_fallback")
    @patch("ingest.finnhub_candles.FinnhubCandleFetcher.fetch")
    async def test_premium_no_fallback_available(self, mock_fetch: AsyncMock, mock_fallback: AsyncMock) -> None:
...
class TestRateLimitRetries:
    ...
    @pytest.mark.asyncio
    async def test_fetch_429_raises_immediately_no_retry(self) -> None:
        """Fetcher should raise immediately on HTTP 429 — no retry (single-attempt strategy)."""
        ...
        class _FakeClient:
            ...
            async def get(self, _url: str, params: dict[str, Any]) -> _FakeResponse:
                self.calls += 1
                # Single-attempt strategy: first call returns 429
