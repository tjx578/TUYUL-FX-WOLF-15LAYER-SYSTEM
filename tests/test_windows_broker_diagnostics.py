...
pytestmark = pytest.mark.skipif(
    os.name != "nt" or not EXE, reason="Requires explicitly selected Windows CI artifact"
)
...
def command(vault: Path, digest: str, pipe: str, mode: str) -> list[str]:
    return [
        str(Path(EXE).resolve()), "--vault", str(vault), "--vault-sha256", digest,
        "--pipe-name", pipe, "--executor-id", SENTINEL,
        "--account-reference", SENTINEL, "--broker-server", SENTINEL,
        "--verification-key-id", SENTINEL, "--account-reference-sha256", "0" * 64,
        "--timeout-ms", "1000", "--serve-mode", mode,
    ]
...
def test_argument_failure_preserves_exit_code_and_reason():
    result = subprocess.run([str(Path(EXE).resolve()), SENTINEL],
...
def test_missing_vault_does_not_disclose_path(tmp_path):
    result = subprocess.run(command(tmp_path / SENTINEL, "0" * 64,
                                    "wolf15-lean-d0-diagnostics", "once"),
                            capture_output=True, text=True, timeout=10)
    ...
    with subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        ...
        try:
            if mode == "persistent":
                ...
                while True:
                    try:
                        client = open("\\\\.\\pipe\\" + pipe, "rb", buffering=0)
                        break
                    except FileNotFoundError:
                        if process.poll() is not None or time.monotonic() >= deadline:
                            raise AssertionError("Synthetic pipe was not available")
                        time.sleep(0.05)
                client.close()
