...
def test_market_closed_never_alarms_even_when_silent():
    ...
    for offset in (0.0, 1_000.0, 100_000.0):
        assert (
            heartbeat.check(freshest_tick_epoch=None, market_open=False, now=1_000_000.0 + offset)
            is None
        )
...
def test_recovery_emits_once_then_stays_quiet():
    ...
    recovered = heartbeat.check(
        freshest_tick_epoch=start + 795.0, market_open=True, now=start + 800.0
    )
    ...
    assert (
        heartbeat.check(freshest_tick_epoch=start + 895.0, market_open=True, now=start + 900.0)
        is None
    )
...
def test_sunday_reopen_grace_holds_friday_stale_tick():
    ...
    assert (
        heartbeat.check(freshest_tick_epoch=friday_tick, market_open=True, now=reopen + 500.0)
        is None
    )
