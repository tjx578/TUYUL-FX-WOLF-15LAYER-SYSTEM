...
def test_eaf_good_trader_3hr_session(analyzer):
    ...
    result = analyzer._inner.analyze("EURUSD", session_hours=3.0)
...
def test_eaf_revenge_trading_scenario(analyzer):
    ...
    assert "consecutive_losses" in result["recommendation"]
    assert "drawdown" in result["recommendation"]
    assert "eaf_below_threshold" in result["recommendation"]


def test_eaf_high_fatigue_scenario(analyzer):
    """Test EAF with high fatigue (6+ hours)."""
    result = analyzer._inner.analyze("EURUSD", session_hours=6.0)
...
def test_eaf_convex_weighted_formula(analyzer):
    ...
    result = analyzer._inner.analyze("EURUSD", session_hours=1.0)
...
def test_eaf_stability_index_no_win_rate_penalty(analyzer):
    ...
    assert analyzer._inner._win_streak == 5
...
def test_eaf_focus_level_180min_threshold(analyzer):
    ...
    result = analyzer._inner.analyze("EURUSD", session_hours=2.5)
    ...
    result = analyzer._inner.analyze("EURUSD", session_hours=3.5)
