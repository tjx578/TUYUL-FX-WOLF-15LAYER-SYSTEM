...
class TestStaleFeedCircuitBreaker:
    ...
    @staticmethod
    def _patch_last_seen(monkeypatch: pytest.MonkeyPatch, value: float | None) -> None:
        monkeypatch.setattr("state.data_freshness.read_authoritative_last_seen_ts", lambda symbol: value)

    def test_no_feed_data_returns_hold(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """No tick ever received → feed is stale → HOLD."""
        self._patch_last_seen(monkeypatch, None)
        engine = _make_engine()
    ...
    def test_stale_feed_returns_hold(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Feed older than threshold → HOLD with circuit_breaker=True."""
        self._patch_last_seen(monkeypatch, time.time() - 200)
    ...
    def test_fresh_feed_proceeds_normally(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Fresh tick within threshold → circuit breaker does NOT fire."""
        self._patch_last_seen(monkeypatch, time.time())
    ...
    def test_circuit_breaker_never_returns_execute(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_circuit_breaker_reason_contains_info(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Reason string should indicate feed staleness and threshold."""
        self._patch_last_seen(monkeypatch, time.time() - 300)
    ...
    def test_no_feed_data_reason_says_no_feed(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_configurable_threshold_respected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """feed_stale_threshold_sec config controls when breaker fires."""
        self._patch_last_seen(monkeypatch, time.time() - 60)
    ...
    def test_circuit_breaker_gate_summary_zeroed(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_circuit_breaker_enrichment_fields_present(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_different_symbols_independent(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "state.data_freshness.read_authoritative_last_seen_ts",
            lambda symbol: time.time() if symbol == "GBPUSD" else None,
        )
