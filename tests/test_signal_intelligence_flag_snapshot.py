...
def test_snapshot_reports_log_noise_governance_defaults(monkeypatch, caplog):
    ...
    for name in (
        ...
        "SIGNAL_PRICE_INTEGRITY_ENABLED",
        "SIGNAL_PRICE_INTEGRITY_GATE_ENABLED",
        "SIGNAL_PRICE_MAX_QUOTE_AGE_SECONDS",
        "DIRECT_ABSORPTION_VALID_ENABLED",
        ...
        "SIGNAL_WATCH_PROMOTION_SUMMARY_INTERVAL_SECONDS",
        ...
        "MICROBOOST_STALE_CLUSTER_TERMINALIZATION_ENABLED",
        "MICROBOOST_STALE_CLUSTER_TERMINAL_CACHE_SIZE",
    ...
    assert p["SIGNAL_WATCH_PRODUCTION_GATE_ENABLED"] is True
    assert p["SIGNAL_WATCH_REQUIRE_DECISION_GRADE_CONTEXT"] is False
    assert p["SIGNAL_PRICE_INTEGRITY_ENABLED"] is True
    assert p["SIGNAL_PRICE_INTEGRITY_GATE_ENABLED"] is True
    assert p["SIGNAL_PRICE_MAX_QUOTE_AGE_SECONDS"] == 5.0
    assert p["DIRECT_ABSORPTION_VALID_ENABLED"] is False
    assert p["SIGNAL_TRADEPLAN_H4_ATR_STOP_BUFFER_RATIO"] == 0.10
    assert p["SIGNAL_TRADEPLAN_SPREAD_STOP_BUFFER_MULTIPLIER"] == 1.5
    assert p["SIGNAL_WATCH_INTERNAL_SUMMARY_ENABLED"] is True
    assert p["SIGNAL_WATCH_INTERNAL_SUMMARY_INTERVAL_SECONDS"] == 300.0
    assert p["SIGNAL_WATCH_PROMOTION_DIAGNOSTIC_PRODUCTION_GATE_ENABLED"] is True
    assert p["SIGNAL_WATCH_PROMOTION_SUMMARY_ENABLED"] is True
    assert p["SIGNAL_WATCH_PROMOTION_SUMMARY_INTERVAL_SECONDS"] == 300.0
    ...
    assert p["MICROBOOST_STALE_CLUSTER_TERMINALIZATION_ENABLED"] is True
    assert p["MICROBOOST_STALE_CLUSTER_TERMINAL_CACHE_SIZE"] == 512.0
    assert p["MICROBOOST_ACTIVE_TIMING_MAX_AGE_SECONDS"] == 120.0
    assert p["MICROBOOST_LINEAGE_WAIT_GRACE_SECONDS"] == 420.0
