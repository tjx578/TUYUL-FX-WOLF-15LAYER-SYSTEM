"""Integration test for L2 soft evidence propagation through L8 into L12."""
...
def _build_upstream(*, l2_layer: dict, l8_layer: dict, phase3_status: str) -> dict:
    return {
        "input_ref": "EURUSD_H1_integration_soft",
        ...
        "upstream_result": {
            "upstream_result": {
                "upstream_result": {
                    "phase_results": {
                        "PHASE_1": {
                            "chain_status": "WARN" if l2_layer.get("status") == "WARN" else "FAIL",
                            "summary_status": {"L1": "PASS", "L2": l2_layer["status"], "L3": "PASS"},
                            "layer_results": {
...
def test_l2_soft_evidence_degrades_l8_and_reaches_l12_as_warning() -> None:
    l2_layer = {
        "status": "WARN",
        "score_numeric": 0.88,
        "evidence_score": 0.52,
    ...
    assert "UPSTREAM_L2_WEAK_EVIDENCE" in l8["warning_codes"]

    l12_input = build_l12_input_from_upstream(
        _build_upstream(l2_layer=l2_layer, l8_layer=l8, phase3_status="WARN"),
    )
    result = L12RouterEvaluator().evaluate(l12_input)

    assert "L2_WEAK_EVIDENCE" in result.warning_codes
    assert "D1_H4_DIRECTION_CONFLICT" in result.warning_codes
    assert L12BlockerCode.L2_HARD_ILLEGALITY.value not in result.blocker_codes
    assert result.audit["l2_evidence"]["status"] == "WARN"
    assert result.audit["l2_evidence"]["hard_stop"] is False