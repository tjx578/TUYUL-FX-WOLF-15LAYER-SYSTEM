...
class FinnhubCandleFetcher:
    ...
    async def fetch(self, symbol: str, timeframe: str, bars: int = 100) -> list[dict[str, Any]]:
        ...
        if timeframe == "M15":
            logger.warning(f"M15 timeframe is built from ticks only, not fetched from REST")
            return []
    ...
    def _aggregate_h4(self, h1_candles: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for h1 in h1_candles:
            ...
            # H4 alignment: floor to 4-hour intervals starting at 00:00 UTC
            h4_hour = (ts.hour // 4) * 4
            h4_start = ts.replace(hour=h4_hour, minute=0, second=0, microsecond=0)
            
            # Check if this H1 belongs to current group
            if current_group:
                first_ts = current_group[0]["timestamp"]
                if isinstance(first_ts, datetime):
                    first_h4_start = first_ts.replace(
                        hour=(first_ts.hour // 4) * 4, 
                        minute=0, 
                        second=0, 
                        microsecond=0
                    )
                    ...
                    first_h4_start = first_dt.replace(
                        hour=(first_dt.hour // 4) * 4,
                        minute=0,
                        second=0,
                        microsecond=0
                    )
                    
                if h4_start != first_h4_start:
    ...
    async def warmup_all(self) -> dict[str, dict[str, list[dict[str, Any]]]]:
        ...
        for symbol in enabled_symbols:
            for tf in timeframes:
                tasks.append(self._warmup_symbol_tf(symbol, tf, warmup_bars, results))
