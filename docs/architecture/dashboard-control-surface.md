...
Its response is the **configured pair inventory**: one row per configured pair, with `count` the number of inventory rows published — not a count of cached verdicts. A pair keeps its row when it has no verdict yet, so an operator can distinguish configured-but-not-ready from not-configured: `snapshot_present: false` is a configured pair with no cached verdict, `warmup_ready: false` a configured pair still warming up, and `active: false` a configured pair that is disabled. `source_ok` is taken from the read-health each reader already tracks rather than from exceptions, because both readers are fail-soft: a Redis failure degrades to a missing verdict and zero warmup bars, which would otherwise be indistinguishable from ordinary warmup. Failure counters are captured before health sampling, so concurrent failures cannot be absorbed into the scan baseline. Verdicts use one MGET and warmup uses one pipelined, bounded LRANGE batch for the configured inventory; no per-pair ping is added. Warmup uses the same canonical minimum counts as the pipeline (including H1=30) and the shared ordered `CANDLE_HISTORY_KEY_PREFIXES` resolver; it selects the first nonempty List namespace for each symbol/timeframe without combining histories. Counts use the same JSON-object acceptance and stale-OHLC run collapse as RedisConsumer; malformed entries and redundant runs cannot satisfy warmup. A selected history exceeding CANDLE_HISTORY_MAXLEN is unavailable rather than silently truncated into a readiness claim. Successful and ordinary early-return pipeline results preserve the assessed governance gate record in the cache payload, separately from reflective rollout metadata. The endpoint is authenticated like its read-only siblings in `api/dashboard_routes.py` and carries no write path.
