...
Its response is the **configured pair inventory**: one row per configured pair, with `count` counting inventory rows rather than cached verdicts. Missing snapshots remain visible with `snapshot_present: false`; disabled pairs have `active: false`.

`warmup_ready` is `boolean | null`, copied from the engine's actual warmup assessment in the cached analysis cycle. `true` and `false` represent that measured result; missing/legacy snapshots and safe-mode cycles that skipped the gate report `null` (NOT_MEASURED). It shares the verdict snapshot's `age_seconds` and `quality`, and is not a live observation of the engine bus. The engine's ordinary and early-return results carry measured warmup evidence through the existing cache builder. Aborted analysis still does not persist a verdict, and timeout/error fallback snapshots do not fabricate readiness.

The API makes one MGET for the configured inventory and obtains source health from the existing verdict reader's cooldown and failure counter. Counters are sampled before health so concurrent failures cannot disappear into the baseline. No Redis context reconstruction, LLEN/LRANGE scan, second reader, or per-pair ping is needed: repaired/synthesized bus candles and startup seed retention are evaluated only by the engine's existing warmup gate. A successful cache miss keeps `source_ok: true`; an unavailable verdict source reports `false`. The endpoint remains authenticated like its read-only siblings and adds no execution authority.
