...
Core `GET /api/v1/dashboard/pair-states` is that settled contract. It composes the per-pair projection on the service side — `symbol`, `verdict`, `admission`, `reason_code`, `age_seconds`, `quality`, `warmup_ready`, `active`, `snapshot_present` — applying the shared verdict normalization in `api/verdict_normalization.py` rather than leaving a client to infer an absent governance action. Only declared values are published: a verdict, admission or reason outside its set is `null`, and no confidence, score, gate, execution or diagnostic field is included. Its response is the available cached-verdict subset, at most one row per configured pair. The endpoint is authenticated like its read-only siblings in `api/dashboard_routes.py` and carries no write path.
